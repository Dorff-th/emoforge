...
@Slf4j
public class EditorImageCleanupService {
    ...
    public void clean() {
        ...
        List<Attachment> garbage = attachmentRepository.findUnusedEditorImages();
        ...
        for (Attachment a : garbage) {
            ...
            // 3) 상대경로면 baseDir과 합쳐서 생성
                ...
                path = Paths.get(imageBaseDir, fileUrl);
            }

            try {
                Files.deleteIfExists(path);
                log.info("Deleted editor image file: {}", path);
            } catch (Exception e) {
                log.error("Failed to delete file: {}", path, e);
            }
        }

        int deleted = attachmentRepository.deleteUnusedEditorImages();
        log.info("DB editor image deleted count = {}", deleted);
    }
