...
const DesktopNav = ({ isAuthenticated }: DesktopNavProps) => {
  ...
  return (
    <nav className="hidden md:flex items-center gap-3 text-sm text-gray-600">
        ...
        {aboutOpen && (
          <>
            ...
            <div className="absolute left-0 top-full w-48 rounded-md border bg-white shadow-lg z-20">
              <NavItem to="/about/emoforge" icon={Layers} label="Emoforge" />
              <NavItem to="/about/intro" icon={Info} label="Intro" />
              <NavItem
                to="/about/analysis"
                icon={TrendingUp}
                label="Analysis"
      ...
      <ExternalLink href={SERVICE_URLS.POST} icon={FileText} label="Posts" />
      {/* Diary – 로그인 상태에서만 */}
      {isAuthenticated && (
        <ExternalLink
