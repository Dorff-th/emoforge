    ...
    async (error) => {
      ...
      if (status === 401) {
        ...
        if (originalRequest.url.includes("/refresh")) {
          console.log("[REFRESH FAILURE] refresh API도 401 → 강제 로그아웃");
        ...
        if (originalRequest._retry) {
          console.log("[STOP] 이미 refresh 시도한 요청 → logout 처리");
          store.dispatch(logoutThunk());
          return Promise.reject(error);
        }
        ...
        try {
          console.log("[TRY REFRESH] refresh_token 요청 시작.");

          await axios.post(`${API_AUTH_BASE_URL}/refresh`, null, {
            withCredentials: true,
          });

          console.log("[REFRESH DONE] 재요청 실행");
          return instance(originalRequest);

        } catch (refreshErr) {
          console.log("[REFRESH FAILED] refresh도 실패 → logout");
          store.dispatch(logoutThunk());
